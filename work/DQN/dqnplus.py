...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, n_width: int = 10, n_height: int = 7, u_size=40, default_reward: float = 0, default_type=0,
        ...
        self.grids = GridMatrix(n_width=self.n_width, n_height=self.n_height, default_reward=self.default_reward,
                                default_type=self.default_type, default_value=0.0)
...
def LargeGridWorld():
    ...
    '''
    env = GridWorldEnv(n_width=10, n_height=10, u_size=40, default_reward=0, default_type=0, windy=False)
    env.start = (0, 9)
    env.ends = [(5, 4)]
...
def SimpleGridWorld():
    ...
    '''
    env = GridWorldEnv(n_width=10, n_height=7, u_size=60, default_reward=-1, default_type=0, windy=False)
    env.start = (0, 3)
    env.ends = [(7, 3)]
    env.rewards = [(7, 3, 1)]
    env.refresh_setting()
...
def WindyGridWorld():
    ...
    '''
    env = GridWorldEnv(n_width=10, n_height=7, u_size=60, default_reward=-1, default_type=0, windy=True)
    env.start = (0, 3)
    env.ends = [(7, 3)]
    env.rewards = [(7, 3, 1)]
...
def SkullAndTreasure():
    ...
    '''
    env = GridWorldEnv(n_width=5, n_height=2, u_size=60, default_reward=-1, default_type=0, windy=False)
    env.action_space = spaces.Discrete(4)  # left or right
    env.start = (0, 1)
    env.ends = [(2, 0)]
...
if __name__ == "__main__":
    ...
    for _ in range(20000):
        ...
        state, reward, isdone, info = env._step(a)
