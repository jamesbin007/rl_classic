...
class MA2C_PNET(MA2C_NC):
    ...
    def _init_policy(self):
        # print('powernet _init_policy',self.identical_agent)
        if self.identical_agent:
            return PowerNetMultiAgentPolicy(self.n_s, self.n_a, self.n_agent, self.n_step,
                                            self.neighbor_mask, n_fc=self.n_fc, n_h=self.n_lstm, device=self.device)
        else:
            return PowerNetMultiAgentPolicy(self.n_s, self.n_a, self.n_agent, self.n_step,
                                            self.neighbor_mask, n_fc=self.n_fc, n_h=self.n_lstm,
                                            n_s_ls=self.n_s_ls, n_a_ls=self.n_a_ls, identical=False, device=self.device)
